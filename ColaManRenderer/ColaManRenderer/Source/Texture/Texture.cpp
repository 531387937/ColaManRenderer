...
void CTexture::LoadHDRTexture(CD3D12Device* Device)
{
	...
	CreateHDRTextureFromFile(CFormatConvert::WStrToStr(FilePath), TextureResource.TextureInfo, InitData, TextureResource.TextureData);

	TextureResource.InitData.push_back(InitData);
}

void CTexture::SetTextureResourceDirectly(const STextureInfo& InTextureInfo, const std::vector<uint8_t>& InTextureData, const D3D12_SUBRESOURCE_DATA& InInitData)
{
