...
TBinaryReader::TBinaryReader(_In_z_ const wchar_t* fileName) noexcept(false) :
    mPos(nullptr),
    mEnd(nullptr)
{
    size_t dataSize;
...
TBinaryReader::TBinaryReader(_In_reads_bytes_(dataSize) const uint8_t* dataBlob, size_t dataSize) noexcept :
    mPos(dataBlob),
