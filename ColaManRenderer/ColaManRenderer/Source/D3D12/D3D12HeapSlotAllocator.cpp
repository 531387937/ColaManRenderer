...
CD3D12HeapSlotAllocator::CD3D12HeapSlotAllocator(ID3D12Device* InDevice, D3D12_DESCRIPTOR_HEAP_TYPE Type,
...
CD3D12HeapSlotAllocator::~CD3D12HeapSlotAllocator()
{
}

D3D12_DESCRIPTOR_HEAP_DESC CD3D12HeapSlotAllocator::CreateHeapDesc(D3D12_DESCRIPTOR_HEAP_TYPE Type,
                                                                   uint32_t NumDescriptorsPerHeap)
{
...
void CD3D12HeapSlotAllocator::AllocateHeap()
{
    ...
    Entry.FreeList.push_back({
        HeapBase.ptr, HeapBase.ptr + static_cast<SIZE_T>(HeapDesc.NumDescriptors) * DescriptorSize
    });
...
CD3D12HeapSlotAllocator::HeapSlot CD3D12HeapSlotAllocator::AllocateHeapSlot()
{
    ...
    if (EntryIndex == -1)
    {
        ...
        EntryIndex = static_cast<int>(HeapMap.size() - 1);
    }
    ...
    HeapSlot Slot = {static_cast<uint32_t>(EntryIndex), Range.Start};
...
void CD3D12HeapSlotAllocator::FreeHeapSlot(const HeapSlot& Slot)
{
