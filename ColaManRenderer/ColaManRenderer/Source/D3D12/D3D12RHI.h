...
public:
    ...
    void CopyBufferRegion(CD3D12Resource* dstResource, UINT64 dstOffset, CD3D12Resource* srcResource, UINT64 srcOffset,
                          UINT64 size);
    ...
    CD3D12TextureRef CreateTexture(const STextureInfo& textureInfo, uint32_t createFlags,
                                   SVector4 rtvClearValue = SVector4::Zero);

    CD3D12TextureRef CreateTexture(Microsoft::WRL::ComPtr<ID3D12Resource> d3dResource, STextureInfo& textureInfo,
...
private:
    void CreateDefaultBuffer(uint32_t size, uint32_t alignment, D3D12_RESOURCE_FLAGS flags,
                             CD3D12ResourceLocation& resourceLocation);

    void CreateAndInitDefaultBuffer(const void* contents, uint32_t size, uint32_t alignment,
                                    CD3D12ResourceLocation& resourceLocation);

    CD3D12TextureRef CreateTextureResource(const STextureInfo& textureInfo, uint32_t createFlags,
