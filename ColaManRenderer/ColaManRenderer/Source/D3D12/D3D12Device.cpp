...
void CD3D12Device::Initialize()
{
    ...
    UploadBufferAllocator = std::make_unique<CD3D12UploadBufferAllocator>(D3DDevice.Get());

    DefaultBufferAllocator = std::make_unique<CD3D12DefaultBufferAllocator>(D3DDevice.Get());

    TextureResourceAllocator = std::make_unique<CD3D3TextureResourceAllocator>(D3DDevice.Get());

    //Create heapSlot allocator
    RTVHeapSlotAllocator = std::make_unique<CD3D12HeapSlotAllocator>(D3DDevice.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
                                                                     200);

    DSVHeapSlotAllocator = std::make_unique<CD3D12HeapSlotAllocator>(D3DDevice.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV,
                                                                     200);

    SRVHeapSlotAllocator = std::make_unique<CD3D12HeapSlotAllocator>(D3DDevice.Get(),
                                                                     D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 200);
}
