...
CD3D12TextureRef CD3D12RHI::CreateTexture(Microsoft::WRL::ComPtr<ID3D12Resource> D3DResource, STextureInfo& TextureInfo, uint32_t CreateFlags)
{
	CD3D12TextureRef TextureRef = std::make_shared<CD3D12Texture>();

	CD3D12Resource* NewResource = new CD3D12Resource(D3DResource, TextureInfo.InitState);
...
CD3D12TextureRef CD3D12RHI::CreateTextureResource(const STextureInfo& TextureInfo, uint32_t CreateFlags, SVector4 RTVClearValue)
{
	CD3D12TextureRef TextureRef = std::make_shared<CD3D12Texture>();
	...
	else
	{
		...
		CD3D12Resource* NewResource = new CD3D12Resource(Resource, TextureInfo.InitState);
		TextureRef->ResourceLocation.UnderlyingResource = NewResource;
		TextureRef->ResourceLocation.SetType(CD3D12ResourceLocation::EResourceLocationType::StandAlone);
	}
