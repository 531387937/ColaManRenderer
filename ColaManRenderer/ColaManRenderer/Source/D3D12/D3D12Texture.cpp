...
CD3D12TextureRef CD3D12RHI::CreateTexture(const STextureInfo& TextureInfo, uint32_t CreateFlags, SVector4 RTVClearValue)
{
    CD3D12TextureRef TextureRef = CreateTextureResource(TextureInfo, CreateFlags, RTVClearValue);
...
CD3D12TextureRef CD3D12RHI::CreateTexture(Microsoft::WRL::ComPtr<ID3D12Resource> D3DResource, STextureInfo& TextureInfo,
                                          uint32_t CreateFlags)
{
    auto TextureRef = std::make_shared<CD3D12Texture>();

    auto NewResource = new CD3D12Resource(D3DResource, TextureInfo.InitState);
    TextureRef->ResourceLocation.UnderlyingResource = NewResource;
    TextureRef->ResourceLocation.SetType(CD3D12ResourceLocation::EResourceLocationType::StandAlone);
...
CD3D12TextureRef CD3D12RHI::CreateTextureResource(const STextureInfo& TextureInfo, uint32_t CreateFlags,
                                                  SVector4 RTVClearValue)
{
    ...
    TexDesc.Height = static_cast<uint32_t>(TextureInfo.Height);
    TexDesc.DepthOrArraySize = (TextureInfo.Depth > 1)
                                   ? static_cast<uint16_t>(TextureInfo.Depth)
                                   : static_cast<uint16_t>(TextureInfo.ArraySize);
    TexDesc.MipLevels = static_cast<uint16_t>(TextureInfo.MipCount);
    ...
    else
    {
        ...
        auto NewResource = new CD3D12Resource(Resource, TextureInfo.InitState);
        TextureRef->ResourceLocation.UnderlyingResource = NewResource;
        TextureRef->ResourceLocation.SetType(CD3D12ResourceLocation::EResourceLocationType::StandAlone);
    }
...
void CD3D12RHI::CreateTextureViews(CD3D12TextureRef TextureRef, const STextureInfo& TextureInfo, uint32_t CreateFlags)
{
    auto TextureResource = TextureRef->GetD3DResource();
    ...
    if (CreateFlags & TexCreate_SRV)
    {
        ...
        if (TextureInfo.Type == ETextureType::TEXTURE_2D)
        {
            ...
            SrvDesc.Texture2D.MipLevels = static_cast<uint16_t>(TextureInfo.MipCount);
        }
        else if (TextureInfo.Type == ETextureType::TEXTURE_CUBE)
        {
            SrvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
            SrvDesc.TextureCube.MostDetailedMip = 0;
            SrvDesc.TextureCube.MipLevels = static_cast<uint16_t>(TextureInfo.MipCount);
        }
        else
        {
            SrvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE3D;
            SrvDesc.Texture3D.MostDetailedMip = 0;
            SrvDesc.Texture3D.MipLevels = static_cast<uint16_t>(TextureInfo.MipCount);
        }

        TextureRef->AddSRV(std::make_unique<CD3D12ShaderResourceView>(GetDevice(), SrvDesc, TextureResource));
    }
    ...
    if (CreateFlags & TexCreate_RTV)
    {
        ...
        TextureRef->AddRTV(std::make_unique<CD3D12RenderTargetView>(GetDevice(), RtvDesc, TextureResource));
    }
    ...
    if (CreateFlags & TexCreate_DSV)
    {
        ...
        TextureRef->AddDSV(std::make_unique<CD3D12DepthStencilView>(GetDevice(), DSVDesc, TextureResource));
    }
    ...
    if (CreateFlags & TexCreate_UAV)
    {
        ...
        TextureRef->AddUAV(std::make_unique<CD3D12UnorderedAccessView>(GetDevice(), UAVDesc, TextureResource));
    }
}

void CD3D12RHI::UploadTextureData(CD3D12TextureRef Texture, const std::vector<D3D12_SUBRESOURCE_DATA>& InitData)
{
    auto TextureResource = Texture->GetResource();
    D3D12_RESOURCE_DESC TexDesc = TextureResource->D3DResource->GetDesc();

    //GetCopyableFootprints
    const UINT NumSubresources = static_cast<UINT>(InitData.size());
    std::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> Layouts(NumSubresources);
    std::vector<uint32_t> NumRows(NumSubresources);
    std::vector<uint64_t> RowSizesInBytes(NumSubresources);

    uint64_t RequiredSize = 0;
    Device->GetD3DDevice()->GetCopyableFootprints(&TexDesc, 0, NumSubresources, 0, &Layouts[0], &NumRows[0],
    ...
    void* MappedData = UploadBufferAllocator->AllocUploadResource(static_cast<uint32_t>(RequiredSize),
    ...
    {
        if (RowSizesInBytes[i] > static_cast<SIZE_T>(-1))
        {
            assert(0);
        }
        D3D12_MEMCPY_DEST DestData = {
            static_cast<BYTE*>(MappedData) + Layouts[i].Offset, Layouts[i].Footprint.RowPitch,
            static_cast<SIZE_T>(Layouts[i].Footprint.RowPitch) * static_cast<SIZE_T>(NumRows[i])
        };
        MemcpySubresource(&DestData, &(InitData[i]), RowSizesInBytes[i], NumRows[i], Layouts[i].Footprint.Depth);
    }
