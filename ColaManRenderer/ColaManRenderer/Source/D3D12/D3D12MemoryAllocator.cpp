...
CD3D12BuddyAllocator::CD3D12BuddyAllocator(ID3D12Device* InDevice, const TAllocatorInitData& InInitData)
    : InitData(InInitData), D3DDevice(InDevice)
{
    Initialize();
}
...
void CD3D12BuddyAllocator::Initialize()
{
    ...
    else //ManualSubAllocation
    {
        ...
        BackingResource = new CD3D12Resource(Resource);
...
bool CD3D12BuddyAllocator::AllocResource(uint32_t Size, uint32_t Alignment, CD3D12ResourceLocation& ResourceLocation)
{
    ...
    if (CanAllocate(SizeToAllocate))
    {
        ...
        ResourceLocation.SetType(CD3D12ResourceLocation::EResourceLocationType::SubAllocation);
        ...
        if (InitData.AllocationStrategy == EAllocationStrategy::ManualSubAllocation)
        {
            ...
            if (InitData.HeapType == D3D12_HEAP_TYPE_UPLOAD)
            {
                ResourceLocation.MappedAddress = (static_cast<uint8_t*>(BackingResource->MappedBaseAddress) +
                    AlignedOffsetFromResourceBase);
            }
        }
        else
        {
...
uint32_t CD3D12BuddyAllocator::GetSizeToAllocate(uint32_t Size, uint32_t Alignment)
{
...
bool CD3D12BuddyAllocator::CanAllocate(uint32_t SizeToAllocate)
{
    ...
    for (int i = static_cast<int>(FreeBlocks.size()) - 1; i >= 0; i--)
    {
...
uint32_t CD3D12BuddyAllocator::AllocateBlock(uint32_t Order)
{
    uint32_t Offset;
...
void CD3D12BuddyAllocator::Deallocate(CD3D12ResourceLocation& ResourceLocation)
{
    DeferredDeletionQueue.push_back(ResourceLocation.BlockData);
}

void CD3D12BuddyAllocator::CleanUpAllocations()
{
    for (int32_t i = 0; i < DeferredDeletionQueue.size(); i++)
    {
        const CD3D12BuddyBlockData& Block = DeferredDeletionQueue[i];
...
void CD3D12BuddyAllocator::DeallocateInternal(const CD3D12BuddyBlockData& Block)
{
...
void CD3D12BuddyAllocator::DeallocateBlock(uint32_t Offset, uint32_t Order)
{
    // Get buddy block
...
CD3D12MultiBuddyAllocator::CD3D12MultiBuddyAllocator(ID3D12Device* InDevice,
                                                     const CD3D12BuddyAllocator::TAllocatorInitData& InInitData)
    : Device(InDevice), InitData(InInitData)
{
}

CD3D12MultiBuddyAllocator::~CD3D12MultiBuddyAllocator()
{
}

bool CD3D12MultiBuddyAllocator::AllocResource(uint32_t Size, uint32_t Alignment,
                                              CD3D12ResourceLocation& ResourceLocation)
{
    ...
    auto Allocator = std::make_shared<CD3D12BuddyAllocator>(Device, InitData);
...
void CD3D12MultiBuddyAllocator::CleanUpAllocations()
{
...
CD3D12UploadBufferAllocator::CD3D12UploadBufferAllocator(ID3D12Device* InDevice)
{
    CD3D12BuddyAllocator::TAllocatorInitData InitData;
    InitData.AllocationStrategy = CD3D12BuddyAllocator::EAllocationStrategy::ManualSubAllocation;
    InitData.HeapType = D3D12_HEAP_TYPE_UPLOAD;
    InitData.ResourceFlags = D3D12_RESOURCE_FLAG_NONE;

    Allocator = std::make_unique<CD3D12MultiBuddyAllocator>(InDevice, InitData);

    D3DDevice = InDevice;
}

void* CD3D12UploadBufferAllocator::AllocUploadResource(uint32_t Size, uint32_t Alignment,
                                                       CD3D12ResourceLocation& ResourceLocation)
{
...
void CD3D12UploadBufferAllocator::CleanUpAllocations()
{
    Allocator->CleanUpAllocations();
}


CD3D12DefaultBufferAllocator::CD3D12DefaultBufferAllocator(ID3D12Device* InDevice)
{
    {
        CD3D12BuddyAllocator::TAllocatorInitData InitData;
        InitData.AllocationStrategy = CD3D12BuddyAllocator::EAllocationStrategy::ManualSubAllocation;
...

    {
        ...
        UavAllocator = std::make_unique<CD3D12MultiBuddyAllocator>(InDevice, InitData);
    }

    D3DDevice = InDevice;
}

void CD3D12DefaultBufferAllocator::AllocDefaultResource(const D3D12_RESOURCE_DESC& ResourceDesc, uint32_t Alignment,
                                                        CD3D12ResourceLocation& ResourceLocation)
{
    if (ResourceDesc.Flags == D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)
    {
        UavAllocator->AllocResource(static_cast<uint32_t>(ResourceDesc.Width), Alignment, ResourceLocation);
    }
    else
    {
        Allocator->AllocResource(static_cast<uint32_t>(ResourceDesc.Width), Alignment, ResourceLocation);
    }
}

void CD3D12DefaultBufferAllocator::CleanUpAllocations()
{
    Allocator->CleanUpAllocations();
}


CD3D3TextureResourceAllocator::CD3D3TextureResourceAllocator(ID3D12Device* InDevice)
{
    CD3D12BuddyAllocator::TAllocatorInitData InitData;
    InitData.AllocationStrategy = CD3D12BuddyAllocator::EAllocationStrategy::PlacedResource;
    InitData.HeapType = D3D12_HEAP_TYPE_DEFAULT;
...
void CD3D3TextureResourceAllocator::AllocTextureResource(const D3D12_RESOURCE_STATES& ResourceState,
                                                         const D3D12_RESOURCE_DESC& ResourceDesc,
                                                         CD3D12ResourceLocation& ResourceLocation)
{
    const D3D12_RESOURCE_ALLOCATION_INFO Info = D3DDevice->GetResourceAllocationInfo(0, 1, &ResourceDesc);

    Allocator->AllocResource(static_cast<uint32_t>(Info.SizeInBytes), DEFAULT_RESOURCE_ALIGNMENT, ResourceLocation);
    ...
    // Create placed resource
    {
        ...
        D3DDevice->CreatePlacedResource(BackingHeap, HeapOffset, &ResourceDesc, ResourceState, nullptr,
                                        IID_PPV_ARGS(&Resource));

        auto NewResource = new CD3D12Resource(Resource);
        ResourceLocation.UnderlyingResource = NewResource;
        ResourceLocation.BlockData.PlacedResource = NewResource;
        // Will delete Resource when ResourceLocation was destroyed
    }
}

void CD3D3TextureResourceAllocator::CleanUpAllocations()
{
    Allocator->CleanUpAllocations();
}
