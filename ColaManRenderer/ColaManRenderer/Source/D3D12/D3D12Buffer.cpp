...
{
	CD3D12ConstantBufferRef ConstantBufferRef = std::make_shared<CD3D12ConstantBuffer>();
...
CD3D12StructuredBufferRef CD3D12RHI::CreateStructuredBuffer(const void* Contents, uint32_t ElementSize, uint32_t ElementCount)
{
	assert(Contents != nullptr && ElementSize > 0 && ElementCount > 0);

	CD3D12StructuredBufferRef StructuredBufferRef = std::make_shared<CD3D12StructuredBuffer>();
...
{
	CD3D12RWStructuredBufferRef RWStructuredBufferRef = std::make_shared<CD3D12RWStructuredBuffer>();
	...
	// Create UAV
	{
		...
		RWStructuredBufferRef->SetUAV(std::make_unique<CD3D12UnorderedAccessView>(GetDevice(), UAVDesc, BufferResource));
	}
...
{
	CD3D12VertexBufferRef VertexBufferRef = std::make_shared<CD3D12VertexBuffer>();
...
{
	CD3D12IndexBufferRef IndexBufferRef = std::make_shared<CD3D12IndexBuffer>();
...
{
	CD3D12ReadBackBufferRef ReadBackBufferRef = std::make_shared<CD3D12ReadBackBuffer>();
	...
	CD3D12Resource* NewResource = new CD3D12Resource(Resource, D3D12_RESOURCE_STATE_COPY_DEST);
...
void CD3D12RHI::CreateDefaultBuffer(uint32_t Size, uint32_t Alignment, D3D12_RESOURCE_FLAGS Flags, CD3D12ResourceLocation& ResourceLocation)
{
...
void CD3D12RHI::CreateAndInitDefaultBuffer(const void* Contents, uint32_t Size, uint32_t Alignment, CD3D12ResourceLocation& ResourceLocation)
{
	//Create default resource
