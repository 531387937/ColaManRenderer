    ...
    D3D12_SUBRESOURCE_DATA& subresource) noexcept
{
    ...
    if (!maxsize)
    {
        maxsize = static_cast<size_t>(D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION);
    }
    ...
    if (format == DXGI_FORMAT_UNKNOWN)
    {
        ...
        {
            DebugTrace(
                "ERROR: WICTextureLoader does not support all DXGI formats (WIC GUID {%8.8lX-%4.4X-%4.4X-%2.2X%2.2X-%2.2X%2.2X%2.2X%2.2X%2.2X%2.2X}). Consider using DirectXTex.\n",
        ...
        if (SUCCEEDED(frame->GetMetadataQueryReader(metareader.GetAddressOf())))
        {
            ...
            if (SUCCEEDED(metareader->GetContainerFormat(&containerFormat)))
            {
                ...
                if (memcmp(&containerFormat, &GUID_ContainerFormatPng, sizeof(GUID)) == 0)
                {
                    ...
                    else if (SUCCEEDED(metareader->GetMetadataByName(L"/gAMA/ImageGamma", &value)) && value.vt ==
                        VT_UI4)
                    {
    ...
    uint64_t rowBytes = (static_cast<uint64_t>(twidth) * bpp + 7u) / 8u;
    uint64_t numBytes = rowBytes * static_cast<uint64_t>(theight);

    if (rowBytes > UINT32_MAX || numBytes > UINT32_MAX)
        return HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW);

    auto rowPitch = rowBytes;
    auto imageSize = numBytes;
    ...
    else if (twidth != width || theight != height)
    {
        ...
        if (memcmp(&convertGUID, &pfScaler, sizeof(GUID)) == 0)
        {
            ...
            hr = scaler->CopyPixels(nullptr, static_cast<UINT>(rowPitch), static_cast<UINT>(imageSize),
                                    decodedData.data());
            if (FAILED(hr))
                return hr;
        }
