...
namespace DirectX::LoaderHelpers
{
    //--------------------------------------------------------------------------------------
        ...
        _Out_opt_ size_t* outNumRows) noexcept
    {
        ...
        {
            rowBytes = ((width + 1u) >> 1) * bpe;
            numBytes = (rowBytes * height) + ((rowBytes * height + 1u) >>
                1);
    ...
    {
        float origAR = static_cast<float>(origx) / static_cast<float>(origy);

        if (origx > origy)
        {
            size_t x;
            for (x = maxsize; x > 1; x >>= 1) { if (x <= targetx) break; }
            targetx = static_cast<UINT>(x);

            float bestScore = FLT_MAX;
            for (size_t y = maxsize; y > 0; y >>= 1)
            {
                float score = fabsf((static_cast<float>(x) / static_cast<float>(y)) - origAR);
                if (score < bestScore)
                {
                    bestScore = score;
                    targety = static_cast<UINT>(y);
                }
            }
        }
        else
